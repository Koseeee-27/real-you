...
export const SCENE_DESCRIPTION =
  '開発チームのグループチャットで、上司が「今日中に対応が必要な不具合」を共有してきた。チームの空気を読みながら、自分はどう動く？';

/**
 * オンボーディング: 「遊び方」セクションの手順。
 * ゲームの実機構（全3ターン / 4択 / 制限時間 / 正解なし）を端的に伝える。
 * アイコンは表示の都合で OnboardingSlides 側（lucide-react）で対応付ける。
 * `id` がアイコンとの対応キー。
 */
export const HOW_TO_PLAY_STEPS = [
  {
    id: 'turns',
    // 全ターン数は TOTAL_TURNS と一致（TOTAL_TURNS はこの定義より後で宣言されるため直書き）。
    title: '全3ターン',
    body: 'チームのチャットが進んでいきます',
  },
  {
    id: 'choose',
    title: '4つから選ぶ',
...
export type HowToPlayStepId = (typeof HOW_TO_PLAY_STEPS)[number]['id'];
