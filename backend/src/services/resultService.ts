...
export const resultService = {
  async getResult(userId: string): Promise<ResultResponse> {
    ...
    if (gameLogs.length < 3) {
      throw {
        status: 400,
        code: ERROR_CODES.INCOMPLETE_GAMES,
        message: "All games must be completed",
      };
    }

    // 分析（analysis/ に委譲）
    const game1Log = gameLogs.find(
      (log) => log.game_type === GAME_TYPES.TERMS_GAME,
    );
    const game2Log = gameLogs.find(
      (log) => log.game_type === GAME_TYPES.AI_CHAT,
    );
    const game3Log = gameLogs.find(
      (log) => log.game_type === GAME_TYPES.GROUP_CHAT,
    );
    ...
    const game1Data: Game1Data | undefined = parseGameLogOrThrow(
      game1Log,
      game1DataSchema,
      "game1",
      userId,
    );
    const game2Data: Game2Data | undefined = parseGameLogOrThrow(
      game2Log,
      game2DataSchema,
      "game2",
      userId,
    );
    const game3Data: Game3Data | undefined = parseGameLogOrThrow(
      game3Log,
      game3DataSchema,
      "game3",
      userId,
