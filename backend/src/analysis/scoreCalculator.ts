...
function calculateGame1(data: Game1Data | undefined): Game1Result {
  ...
  const duration =
    (lastEvent?.timestamp || 1) - (scrollEvents[0]?.timestamp || 0);
  ...
  const checkboxChanged = Object.values(data.checkboxStates).filter(
    (c) => c.changed,
  ).length;
  ...
  const caution = Math.round(
    sTime * 0.3 + sScroll * 0.25 + sHover * 0.25 + sCheckbox * 0.2,
  );

  //論理性: 再確認行動・逆行スクロール・ポップアップ処理
  const hiddenMatch =
    data.hiddenInput === "確認済み" ? 1 : data.hiddenInput ? 0.5 : 0;
...
function calculateGame2(data: Game2Data | undefined): Game2Result {
  ...
  //論理性: 論理接続詞・不要語
  const logicWords = (
    fullText.match(/なぜなら|つまり|しかし|なので|というのも/g) || []
  ).length;
...
function calculateGame3(data: Game3Data | undefined): Game3Result {
  ...
  const reactionVariance =
    stages.reduce(
      (a, s) => a + Math.pow((s.reactionTimeMs ?? 0) - avgReact, 2),
      0,
    ) / (stages.length || 1);
  ...
  const cooperativeness = Math.round(
    sConform * 0.5 + sWait * 0.3 + sHover * 0.2,
  );
...
export function generateAnalysisResult(
  ...
  const phaseSummaries = buildPhaseSummaries(game1Raw, game2Raw, game3Raw, {
  ...
  return {
    ...
    game_breakdown: {
      ...
      game_2: {
        positivity: g2.positivity,
        calmness: g2.calmness,
        logic: g2.logic,
      },
      game_3: {
