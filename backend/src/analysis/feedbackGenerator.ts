...
export function generateFeedback(
  ...
  const feedbackText: Record<
    string,
    {
      over: { title: string; desc: string };
      under: { title: string; desc: string };
    }
  > = {
    caution: {
      over: {
    ...
    calmness: {
      over: {
        title: "隠れパニックメーカー",
        desc: "あなたは冷静なつもりでも、想定外の事態が起きると無意識に焦りが行動（操作）に表れてしまうようです。",
      },
      under: {
        title: "氷のメンタル",
    ...
    logic: {
      over: {
        title: "直感ドリブン",
        desc: "あなたは論理的に考えているつもりでも、いざという時はフィーリング（直感）で決定を下すことが多いようです。",
      },
      under: {
        title: "隠れた知性",
    ...
    cooperativeness: {
      over: {
        title: "孤高のソロプレイヤー",
        desc: "あなたは協調的だと思っていましたが、実際には周囲に流されず自分の意見を貫く強さ（あるいは頑固さ）を持っています。",
      },
      under: {
        title: "究極のバランサー",
        desc: "あなたは自分を独立心が強いと思っていましたが、実際には無意識に周囲の空気を読み、調和を重んじています。",
      },
    },
    positivity: {
      over: {
        title: "石橋を叩き割る守護者",
        desc: "あなたは積極的だと思い込んでいますが、無意識のうちに失敗を恐れ、行動が遅れる傾向があります。",
      },
      under: {
        title: "前のめりな挑戦者",
        desc: "あなたは自分を消極的だと思っていましたが、チャンスがあれば誰よりも早く（食い気味に）行動を起こしています。",
      },
    },
  };
  ...
  const selectedFeedback = isOverestimated
    ? feedbackText[gapKey].over
    : feedbackText[gapKey].under;
